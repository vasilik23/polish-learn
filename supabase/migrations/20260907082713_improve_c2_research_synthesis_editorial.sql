-- Rerunnable content-only editorial update for C2 research synthesis.
update public.flashcards f set example=v.example from (values
...
) v(id,example) where f.id=v.id;

update public.questions q set explanation=v.explanation from (values
...
) v(lesson_id,position,explanation) where q.lesson_id=v.lesson_id and q.position=v.position;
