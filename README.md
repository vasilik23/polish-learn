...
- **Later:** offline-очередь подключена к урокам слов, повторения, quiz и grammar через
  cookie-only CSRF-protected handoff к идемпотентному API. Полный offline → online
  recovery успешно проверен в production; перед дальнейшим расширением нужен
  период наблюдения. Затем — offline-кэш и нативные клиенты
  iOS/Android. Уведомления — только с
  явным согласием. API уже покрывает каталог, уроки, дневной план,
  профиль, прогресс, черновики, чтение, новости, словарь и SM-2.
