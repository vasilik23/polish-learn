...
| `/history/` | недельный разбор, адаптивная аналитика за 28 дней и постраничная история уроков |
| `/mistakes/` | личная работа над ошибками и целевой тренажёр без хранения выбранных ответов |
| `/notes/` | поиск по личным заметкам к урокам и управление ими |
| `/collections/` | собственные подборки из сохранённых уроков и текстов |
| `/lesson/<id>/` | урок с прогрессом, закладкой и личной синхронизируемой заметкой |
...
Состояние server-rendered урока (текущий шаг и счёт) подписывается Django,
ограничено по времени и привязано к пользователю, уроку и типу задания.
Подмена скрытых полей браузера не может увеличить сохранённый результат.
