...
- **Now:** внутренний измеримый проход C1–C2 завершён; в B1 все 12 тем получили
  regression-выборку объяснений, неоднозначные distractors исправлены, а позиции
  правильных ответов распределены детерминированно. В B2 ответы распределены
  поровну, исправлены 24 слабых набора и проверены объяснения всех двенадцати
  тем. Следующий контентный рубеж — независимая польскоязычная проверка A1–A2.
  Реестр дефектов и roadmap обновляются вместе с каждым проверенным пакетом.
- **Next:** калибровка диагностики и заданий с редактором, развитие аудирования
  до связной речи B2, калибровка обратной связи для письма и сценариев
  online interaction/медиации, а также пилот уже формализованного manifest-
  workflow: из одобренных checksum и model resolutions создаётся безопасный
  review-only preview парных миграций вне migration-каталогов; пилот на реальной
  теме подтвердил детерминизм без записи в БД. Promotion receipt и повторная
  pre-promotion проверка уже связывают approval, checksum и имена миграций;
  следующий шаг — первый ручной выпуск по этой квитанции.
  ...
  Адаптивное закрепление уже добавляет в «Сегодня» не более одного слабого
  недавнего урока с прозрачной причиной, не вытесняя основной урок и не выдавая
  результат за AI/CEFR-диагноз. При недоступной истории сохраняется обычный
  линейный план.
- **Shipped foundation:** offline-очередь подключена к урокам слов, повторения, quiz и grammar через
  ...
  result sync, bootstrap/«Сегодня»/профиль/прогресс/историю, черновики,
  чтение/закладки, новости, словарь/SM-2, аудирование, interaction/медиацию,
  диагностику, письмо, достижения и feedback.
- **Later:** после периода наблюдения — ограниченный авторизованный offline-кэш
  и общий клиент iOS/Android. Для будущих уведомлений уже есть выключенные по
  умолчанию owner-scoped настройки; реальная отправка появится только после
  отдельного согласия, проверки доставки и возможности полного отключения.
