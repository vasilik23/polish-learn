...
class C2CriticalPolemicsEditorialTests(TestCase):
    def test_templates_are_replaced(self):
        cards=Flashcard.objects.filter(id__regex=r"^c23-")
        self.assertEqual(cards.count(),15)
        for card in cards:
                self.assertNotIn("W redagowanym tekście świadomie",card.example)
                self.assertGreaterEqual(len(card.example.split()),10)
        questions=Question.objects.filter(lesson_id="c23-quiz")
