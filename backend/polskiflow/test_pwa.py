...
class PwaPrototypeTests(SimpleTestCase):
    def test_manifest_describes_root_scoped_standalone_app(self):
        ...
        self.assertTrue(all(icon["src"].endswith("?shell=v12") for icon in manifest["icons"]))
    ...
    def test_service_worker_only_precaches_public_shell_assets(self):
        ...
        self.assertIn('const OFFLINE_URL = "/offline/?shell=v12"', source)
        self.assertIn('"/static/polskiflow/app.css?shell=v12"', source)
        self.assertIn('"/static/polskiflow/favicon.svg?shell=v12"', source)
    ...
    def test_base_template_requests_the_versioned_public_assets(self):
        ...
        self.assertContains(response, "/static/polskiflow/app.css?shell=v12")
        self.assertContains(response, "/static/polskiflow/favicon.svg?shell=v12")
