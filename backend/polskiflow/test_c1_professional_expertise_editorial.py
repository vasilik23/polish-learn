...
class C1ProfessionalExpertiseEditorialTests(TestCase):
    def test_flashcards_use_terms_in_natural_professional_contexts(self):
        cards = Flashcard.objects.filter(id__regex=r"^c16-").order_by("position")
    ...
    def test_all_explanations_teach_concepts_or_point_to_reading_evidence(self):
        questions = Question.objects.filter(
            lesson__topic_id="c1-professional-expertise"
        )
        ...
        feasibility = questions.get(lesson_id="c16-grammar", position=3)
        self.assertIn("zasobach, czasie, technologii", feasibility.explanation)
        evidence = questions.get(lesson_id="c16-reading-check", position=0)
        self.assertIn("punkt decyzyjny", evidence.explanation)
