...
class C2ContentTests(TestCase):
    topic_expectations = (
        ...
        ("c2-cross-genre", "c25", 4, 61),
        ("c2-cultural-interpretation", "c26", 5, 62),
        ("c2-expert-mediation", "c27", 6, 63),
        ("c2-research-synthesis", "c28", 7, 64),
        ("c2-authorial-voice", "c29", 8, 65),
        ("c2-capstone-project", "c210", 9, 66),
    )

    def test_course_is_a_curriculum_target_with_ten_vertical_topics(self):
...
class C2ReadingRoutesTests(TestCase):
    ...
    def test_readings_link_to_their_comprehension_lessons(self):
        for prefix in ("c21", "c22", "c23", "c24", "c25", "c26", "c27", "c28", "c29", "c210"):
    ...
    def test_reading_catalog_offers_c2_tab_and_filters_c2_texts(self):
        ...
        self.assertContains(response, "Межжанровая трансформация")
        self.assertContains(response, "Интерпретация культуры")
        self.assertContains(response, "Экспертная медиация")
        self.assertContains(response, "Исследовательский синтез")
        self.assertContains(response, "Авторский голос")
        self.assertContains(response, "Проект C2")
